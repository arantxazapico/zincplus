...
We proceed to describe an algebraic  constraint over $\QQ[X]$ modulo prime ideals, $\REL_{\gp, \megaQ}$, encoding the computation $f^{2^{20}-1}(w_1)$.
...
\end{align*}
The $0$'s at the end of $\uu', \phi(\uu), \phi(\uu')$ can be chosen arbitrarily: our computation never accesses these entries of the vectors, and so these entries are not be subject to any constraint whatsoever.
...
for some $\mu_i(X)\in \ZZ[X]$, for all $i \in [2^{20}-1]$. Above, the operations and equality on the right-hand side  are all happening in the ring $\QQ[X]$.  Similarly, for all $i\in [2^{20}-1]$,
%
\begin{align*}
...
$$
Finally, we select the tuple $\megaQ$ as follows. We let $\megaQ=\{(\cQ_1, I_1), (\cQ_2, I_2), (\cQ_3, I_3), (\cQ_4, I_5)\}$,
where for all $i\in [2^{20}-1]$ (recall we identify $\{0,1\}^{20}$ and $[2^{20}]$)
%
\begin{align*}
    &I_1 = \{0\}, &&\cQ_1=(Q_{1i})_{i\in [2^{20}]},\\
    &I_2 = I_{GF}, &&\cQ_2=(Q_{2i})_{i\in [2^{20}-1]},\\
    &I_3 =(X-2), &&\cQ_3=(Q_{3i})_{i\in [2^{20}-1]},\\
      &I_4 =(X-2), &&\cQ_4=(Q_{4i})_{i\in [2^{20}]},
\end{align*}
with
\begin{align*}
    Q_{1i}(\vv_{\inp,\wit}(X))&=f_{\uu'}(X; i) - f_{\uu}(X;i)\cdot f_{\uu}(X;i) - 2^{32} \mu_i(X)\\
    Q_{2i}(\vv_{\inp,\wit}(X))&=f_{\phi(\uu)}(X; i+1) - (f_{\phi(\uu')}(X;i) + f_{\phi(\uu)}(X;i)) + 2\cdot \eta_i(X),\\
    Q_{3i}(\vv_{\inp,\wit}(X))&=f_{\phi(\uu)}(X;i)-f_{\uu}(X;i),\\
    Q_{4i}(\vv_{\inp,\wit}(X))&=f_{\phi(\uu')}(X;i)-f_{\uu'}(X;i)
\end{align*}
We also want to make sure that $f_{\uu}(X; 1)= w_1$.

Putting everything together, we obtain the following relation for the computation $f^{2^{20}-1}(w_0)$:
   \begin{equation*}
    \begin{aligned}
    &\REL_{\gparams,\megaQ,\cS} =\\ &\left\{ \left(\begin{aligned}&\idx,\\ &\inp;\\ &\wit\end{aligned}\right) \left| \ \begin{aligned}
        &\textbf{Index-instance-witness}\\
    &\quad \idx = \emptyset, \ \YY=(Y_1,\ldots, Y_{20}), \inp =(w_0),\\    
        ...
        &\textbf{Algebraic constraints mod ideal}\\
        &\quad f_{\uu'}(X; i) - f_{\uu}(X;i)\cdot f_{\uu}(X;i) - 2^{32} \mu_i(X) = 0 \text{ for all } i\in \{0,1\}^{20},\\
        &\quad f_{\phi(\uu)}(X; i+1) - (f_{\phi(\uu')}(X;i) + f_{\phi(\uu)}(X;i)) + 2\cdot \eta_i(X) \in I_{GF} \text{ for all } i\in \{0,1\}^{20},\\
        &\quad f_{\phi(\uu)}(X;i)-f_{\uu}(X;i) \in (X-2)\text{ for all } i\in \{0,1\}^{20},\\
        &\quad f_{\phi(\uu')}(X;i)-f_{\uu'}(X;i) \in (X-2)\text{ for all } i\in \{0,1\}^{20},\\
        &\textbf{Membership constraints}\\
