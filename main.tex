...
f(X;\yy), f'(X;\yy) \in [0,2^{32}-1], \quad g(X;\yy), g'(X;\yy) \in \{0,1\}^{<32}[X].
\end{equation}
%
For ease of notation, in what follows we represent elements from $\{0,1\}^{20}$ as integers in the range $[0, 2^{20}-1]$. 

Assuming the membership constraints \cref{e: example_membership_constraints} hold, we have that $f'(X; i)= f(X;i)\cdot f(X;i) \mod 2^{32}$ if and only if 
%
$$
f'(X; i)= (f(X;i)\cdot f(X;i) \mod 2^{32}) \quad \Leftrightarrow  f'(X; i)= f(X;i)\cdot f(X;i) + 2^{32} \mu_i(X)
$$
...
%
$$
g(X; i+1)= g'(X;i)\XOR g(X;i) \quad \Leftrightarrow \quad  g(X; i+1) - (g'(X;i) + g(X;i)) + 2\cdot \eta_i(X) \in I_{GF}
$$
...
%
$$
f(X; i) = g(X; i) \quad \Leftrightarrow\quad f(X;i)-g(X;i) \in (X-2) 
$$
