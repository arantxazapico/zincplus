...
Let $\cQ$ be a set of pairs of the for $(Q, I)$ where $Q$ is a polynomial with coefficients in $\QQ[X]$ (possibly multivariate and of arbitrary degree), and $I$ is a prime ideal of the ring $\QQ[X]$ generated by a non-constant \albert{If it was constant non-zero then the ideal would the whole $\QQ[X]$, so maybe we don't need to ask for this} irreducible polynomial \albert{New wrt to Zinc!}.  We assume $|\cQ|$ is a power of two and index the entries in $\cQ$ with elements from $\{0,1\}^{\log|\cQ|}$.
%
%Let $\cQ$ be a set of polynomials with coefficients in $\QQ[X]$ (possibly multivariate and of arbitrary degree). 
Let $\gp = (k, m, n, \mu, B)$ be global parameters, where $k,m,n,B$ are size parameters. %Let $\evalconstraints$ be a set of triples $(i, j, a)$ where $i,j\in [k]$ and $a \in \QQ$. 
An algebraic relation for $(\gp,\cQ, \evalconstraints)$ is a set $\REL_{\gp, \cQ}$ of triples $(\idx, \inp;\wit)$ with the following properties:
\begin{itemize}
	...
	\item For each  polynomial-ideal pair $(Q,I)\in \cQ$, we have that $Q$ belongs to $I$  when evaluated on the values $$\vv_{\idx, \inp,\wit}(X)=(\yy(X), (g_1(X;\yy),\ldots, g_n(X;\yy), f_1(X,\yy),\ldots, f_k(X;\yy))_{\yy\in \BB^\mu}),$$
    %i.e.
    %$$
    ...
    Formally, $\REL_{\gparams,\cQ}$ has the following form:
    %
    \begin{equation*}
    \begin{aligned}
    \REL_{\gparams,\cQ} = \left\{ (\idx, \inp ; \wit) \left| \ \begin{aligned}
    & \gparams=(k,m,n,\mu, B),  \\
    ...
    \begin{definition}[Strictly algebraic relation $\REL_{\gp, \cQ}$]\label{d: strictly_algebraic} Let $\cQ$ be a set of polynomial-ideal pairs $(Q,I)$ as before. We say that $\cQ$ is \emph{strictly algebraic} if $I$ is the zero ideal for all $(Q,I)\in I$.  In that case, the constraints $Q(\vv_{\idx, \inp,\wit})\in I$ from $\REL_{\gp,\cQ}$ are equivalent to algebraic constraints $(\vv_{\idx, \inp,\wit})=0$.
    \end{definition}
