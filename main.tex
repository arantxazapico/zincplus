...
%
    ...
    & \inp =(\yy(X), \oracle{f_1(X;\YY)}, \ldots, \oracle{f_k(X;\YY)}),\\ &\yy(X)\in \mainQpolyring^{m},\\    
        &\wit = (f_1(X;\YY), \ldots, f_k(X;\YY)) \in \left(\mainQpolyringmultilin[\YY]\right)^{k},\\
...
\end{align*}
%
where the operation $w\cdot w \mod 2^{32}$ here means squaring $w$ and taking as a result $w'$ the reduction of $w^2$ modulo $2^{32}$ in the range $[0,2^{32}-1]$. Then,  $w'\xor w$ means performing the bitwise XOR operation between $w'$ and $w$ when $w'$ and $w$ are represented as strings of $32$ bits. After the bitwise operation is performed, the resulting string of $32$ bits $w''$ is reinterpreted as an integer in the range $[0, 2^{32}-1]$. 
...
\begin{align*}
    ...
    \hat{w_{i+1}} &= \left(\phi(w_i') \xor \phi(w_i)\right) \in \{0,1\}^{\leq 31}[X]\\
     ...
     \phi(\ww') &= (\phi(w_0'),\ldots, \phi(w_{2^{20}-1}'))\in (\{0,1\}^{< 32}[X])^{2^{20}},\\
     \hat{\ww} &= (0,\hat{w_1}, \ldots, \hat{w_{2^{20}-1}}))\in (\{0,1\}^{< 32}[X])^{2^{20}}
\end{align*}
We introduce $4$ multilinear polynomials on $20$ variables $\YY=(Y_1,\ldots, Y_{20})$, with coefficients in $\QQ[X]$. We denote them by $f(X; \YY), f'(X;\YY), g(X;\YY), g'(X;\YY)$. These polynomials will be the witness of the index-instance-witness triple we will build. Intuitively, $f(X; \YY), f'(X;\YY), g(X;\YY), g'(X;\YY)$ are supposed to be the multilinear extensions of the vectors $\ww, \ww', \phi(\ww)$, and $\phi(\ww')$, respectively. 
...
f_1(X;\yy), f_1'(X;\yy) \in [0,2^{32}-1], \quad f_2(X;\yy), f_2'(X;\yy) \in \{0,1\}^{<32}[X].
\end{equation}
%
For ease of notation, in what follows we represent elements from $\{0,1\}^{20}$ as integers in the range $[0, 2^{20}-1]$. 

Assuming the membership constraints \cref{e: example_membership_constraints} hold, we have that $f_1'(X; i)= f_1(X;i)\cdot f_1(X;i) \mod 2^{32}$ if and only if 
%
$$
f_1'(X; i)= (f_1(X;i)\cdot f_1(X;i) \mod 2^{32}) \quad \Leftrightarrow  \quad f_1'(X; i)= f_1(X;i)\cdot f_1(X;i) + 2^{32} \mu_i(X)
$$
for some $\mu_i(X)\in \ZZ[X]$, for all $i=0,\ldots, 2^{20}-1$. Above, the operations and equality on the right-hand side  are all happening in the ring $\QQ[X]$.  Similarly, for all $i\in [1, 2^{20}]$ \albert{inconsistency of subindices?},
%
$$
f_2(X; i+1)= f_2'(X;i)\xor f_2(X;i) \quad \Leftrightarrow \quad  f_2(X; i+1) - (f_2'(X;i) + f_2(X;i)) + 2\cdot \eta_i(X) \in I_{GF}
$$
for some $\eta_i \in \ZZ[X]$,  where $I_{GF}$ is a fixed ideal generated by  a polynomial of degree $32$ which is irreducible both in $\FF[X]$ and in $\FF_2[X]$ \albert{needs more detail, maybe in a separate section}.

Further, as before assuming that \eqref{e: example_membership_constraints}, we have that for all $i\in [0, 2^{20}-1]$  \albert{turn into its own lemma}
%
$$
f_2(X; i) = \phi(f_1(X; i)) \quad \Leftrightarrow\quad f_2(X;i)-f_1(X;i) \in (X-2) 
$$
where $(X-2)$ denotes the ideal of $\QQ[X]$ generated by the polynomial $X-2$. And similarly,
$$
f_2'(X; i) = \phi(_1'(X; i)) \quad \Leftrightarrow\quad f_2'(X;i)-f_1'(X;i) \in (X-2). 
$$
%
% 
We now have all we need to put together a relation of the form $\REL_{\gp, \megaQ, \cS}$ which encodes the computation $f^{2^{20}-1}(w_0)$.

We set $\gp=(k=6,m=1,n=0,\mu=20, B)$ \albert{what $B$?}. The $k=6$ witness polynomials are $$\wit=(f_1(X;\YY), f_1'(X;\YY), f_2(X;\YY),  f_2'(X;\YY), \mu(X;\YY), \eta(X;\YY)),$$ where $\mu(X;\YY), \eta(X;\YY)$ satisfy $\mu(X;i)=\mu_i(X)$ and $\eta(X;i)=\eta_i(X)$ for each $i\in \{0,1\}^{20}$. Then we set $\cS=(S_1, \ldots, S_6)$ with
%
...
\end{align*}
   ...
   \begin{equation*}
    ...
    & \idx = \emptyset, \ \YY=(Y_1,\ldots, Y_{20}), \inp =(w_0),\\    
        &\wit = \left(\begin{split}&f_1(X;\YY), f_1'(X;\YY),\\ &f_2(X;\YY),  f_2'(X;\YY),\\ &\mu(X;\YY), \eta(X;\YY)\end{split}\right) \in \left(\mainQpolyringmultilin[\YY]\right)^{6},\\
        &\textbf{Algebraic constraints mod ideal}\\
        &\quad f_1'(X; i) - f_1(X;i)\cdot f_1(X;i) - 2^{32} \mu_i(X) = 0 \text{ for all } i\in \{0,1\}^{20},\\
        &\quad f_2(X; i+1) - (f_2'(X;i) + f_2(X;i)) + 2\cdot \eta_i(X) \in I_{GF} \text{ for all } i\in \{0,1\}^{20},\\
        &\quad f_2(X;i)-f_1(X;i) \in (X-2)\text{ for all } i\in \{0,1\}^{20},\\
        &\quad f_2'(X;i)-f_1'(X;i) \in (X-2)\text{ for all } i\in \{0,1\}^{20},\\
        &\textbf{Membership constraints}\\
        &\quad  f_1(X;i), f_1'(X;i) \in [0,2^{32}-1], \text{ for all } i \in \{0,1\}^{20},\\ 
        &\quad f_2(X;i), f_2'(X;i) \in \{0,1\}^{<32}[X],  \text{ for all } i \in \{0,1\}^{20}
        %& f_i(a;\yy)=f_j(a;\yy)  \text{ for all } \yy\in \{0,1\}^\mu \text{ and } (i,j,a)\in \evalconstraints
    \end{aligned} \right.\right\}
    \end{aligned}
    \end{equation*}

    Overall, there are $6$ witness multilinear polynomials on $20$ variables, $4\cdot 2^{20}$ algebraic constraints modulo ideals (with the last $2\cdot 2^{20}$ of them being batchable into $2^{20}$ constraints --cf.\ \cref{?}), and $4$ membership constraints for $4$ of the $6$ witness polynomials. 
